...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
	<div class="card">
		<div class="body">
			...
			<form id="form_validation" method="POST" action="{{url('admin/gallery')}}" enctype="multipart/form-data">
				{{csrf_field()}}
...
{{-- 				<label>Title</label>
				<div class="row clearfix">
					<div class="col-sm-12">
						<div class="form-group ">
							<div class="form-line">
								<input type="text" value="{{old('title')}}" name="title" maxlength="50" class="form-control" required>
							</div>
						</div>
					</div>
				</div> --}}
				...
				<div class="col-sm-12 image_class">
					<div class="form-group image_class">
						...
						<input id="ImageInput" type="file" multiple style="max-width:75px; max-height:70px; overflow:hidden;cursor:pointer;font-size: 5em;" accept="image/*" name="image[]" class="col-indigo glyphicon glyphicon-picture fa-5x image_class">
...
<script>
	$(function() {
		var imageWidth = 1170;
		var imageHeight = 509;
		var storageLocation= "{{App\Models\Gallery::IMAGE_LOCATION}}";
		var postUrl = "{{url('admin/gallery/upload-image')}}";
			...
			if (this.files && this.files[0]) {
				$.each(this.files,function(index, value){
					...
					reader.onload = function(e) {
						result.append('<div class="col-md-3 m-t-30" style="min-height:100px"><span><img class="img-responsive" src="' + e.target.result + '"></span></div>');
					}
		...
		function ajaxCall(uploadedImage) {
					$.ajax({
						url : postUrl,
						type: "POST",
						data:  {
					...
					// Show
					result.append('<div id="image-preview-'+data.filename.slice(0,-4)+'" class="col-md-3 m-t-30" style="min-height:100px"><span><img class="img-responsive" src="' + data.src + '"></span></div>');
