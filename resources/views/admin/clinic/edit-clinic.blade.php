...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="card">
			<div class="body">
				<form id="form_validation" method="POST" action="{{url('admin/clinic-update/'.$clinic->id)}}" enctype="multipart/form-data">
					{{csrf_field()}}
					{{ method_field('PUT') }}
					...
					<input type="hidden" name="id" value="{{$clinic->id}}" class="form-control">
					...
					<div class="row clearfix">
						<div class="col-sm-12">
							<div class="form-group ">
								<div class="form-line">
									<input type="text" value="{{$clinic->name}}" maxlength="255" name="title" class="form-control" required>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="row clearfix">
						<div class="col-sm-12">
							<div class="form-group">
								<div class="form-line">
									<textarea name="description"  class="form-control" required>{{$clinic->description}}</textarea>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="col-sm-12 toggle-image" {{$clinic->video!=NULL?' style=display:none ':''}}>
						<div class="form-group">
							...
							<div id="result" class="row">
								@if(null != $clinic->image)
								<div id="image-preview-{{substr($clinic->image,0,-4)}}" class="col-md-3 m-t-20" style="min-height:130px"><span><img class="img-responsive" src="{{url(App\Models\Clinic::IMAGE_LOCATION)."/".$clinic->image}}"></span></div>
								@endif
							</div>
						</div>
					</div>
	...
	<script>
    	...
    	$('#CropperModal').on('shown.bs.modal', function() {
    		...
    		var location= "{{App\Models\Clinic::IMAGE_LOCATION}}";

    		$.ajax({
    			url : "{{url('admin/clinic/upload-image')}}",
    			type: "POST",
    			data:  {
				...
				// Show
				result.html('<div id="image-preview-'+data.filename.slice(0,-4)+'" class="col-md-3 mt-20" style="min-height:100px"><span><img class="img-responsive" src="' + data.src + '"></span></div>');
